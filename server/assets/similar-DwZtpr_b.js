...
function Similar() {
  ...
  const [replies, setReplies] = reactExports.useState(DEFAULT_REPLIES);
  reactExports.useEffect(() => {
    ...
    if (userInput) {
      ...
      console.log("是否包含努力无果隐形天花板关键词:", hasEffortNoResultKeywords);
      ...
      console.log("是否包含迷茫关键词:", hasConfusionKeywords);
      ...
      console.log("是否包含人际困扰关键词:", hasInterpersonalKeywords);
      ...
      console.log("是否包含分手失落关键词:", hasBreakupKeywords);
      ...
      console.log("是否包含期末压力崩溃关键词:", hasExamStressKeywords);
      ...
      console.log("是否包含治愈积极情绪关键词:", hasPositiveKeywords);
      ...
      console.log("是否包含自我否定低自尊关键词:", hasSelfNegationKeywords);
      ...
      console.log("是否包含孤独自洽关键词:", hasSolitudeKeywords);
      ...
      console.log("是否包含友情疏离感关键词:", hasFriendshipKeywords);
      ...
      console.log("是否包含价值观冲突关键词:", hasValueConflictKeywords);
      ...
      console.log("是否包含家庭变故学业打击关键词:", hasFamilyCrisisKeywords);
      ...
      console.log("是否包含挫败权威伤害关键词:", hasFrustrationKeywords);
      ...
      console.log("是否包含逃离自由向往关键词:", hasEscapeKeywords);
      ...
      console.log("是否包含自我成长内在力量关键词:", hasGrowthKeywords);
      ...
      console.log("是否包含野心动力搞钱关键词:", hasAmbitionKeywords);
      ...
      console.log("是否包含怀旧回忆关键词:", hasNostalgiaKeywords);
      if (hasEffortNoResultKeywords) {
        const effortNoResultReplies = EFFORT_NO_RESULT_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置努力无果隐形天花板留言:", effortNoResultReplies);
        setReplies(effortNoResultReplies);
      } else if (hasConfusionKeywords) {
        const confusionReplies = CONFUSION_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置迷茫留言:", confusionReplies);
        setReplies(confusionReplies);
      } else if (hasBreakupKeywords) {
        const breakupReplies = BREAKUP_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置分手失落留言:", breakupReplies);
        setReplies(breakupReplies);
      } else if (hasExamStressKeywords) {
        const examStressReplies = EXAM_STRESS_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置期末压力崩溃留言:", examStressReplies);
        setReplies(examStressReplies);
      } else if (hasPositiveKeywords) {
        const positiveReplies = POSITIVE_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置治愈积极情绪留言:", positiveReplies);
        setReplies(positiveReplies);
      } else if (hasSelfNegationKeywords) {
        const selfNegationReplies = SELF_NEGATION_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置自我否定低自尊留言:", selfNegationReplies);
        setReplies(selfNegationReplies);
      } else if (hasSolitudeKeywords) {
        const solitudeReplies = SOLITUDE_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置孤独自洽留言:", solitudeReplies);
        setReplies(solitudeReplies);
      } else if (hasFriendshipKeywords) {
        const friendshipReplies = FRIENDSHIP_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置友情疏离感留言:", friendshipReplies);
        setReplies(friendshipReplies);
      } else if (hasValueConflictKeywords) {
        const valueConflictReplies = VALUE_CONFLICT_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置价值观冲突留言:", valueConflictReplies);
        setReplies(valueConflictReplies);
      } else if (hasFamilyCrisisKeywords) {
        const familyCrisisReplies = FAMILY_CRISIS_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置家庭变故学业打击留言:", familyCrisisReplies);
        setReplies(familyCrisisReplies);
      } else if (hasFrustrationKeywords) {
        const frustrationReplies = FRUSTRATION_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置挫败权威伤害留言:", frustrationReplies);
        setReplies(frustrationReplies);
      } else if (hasEscapeKeywords) {
        const escapeReplies = ESCAPE_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置逃离自由向往留言:", escapeReplies);
        setReplies(escapeReplies);
      } else if (hasGrowthKeywords) {
        const growthReplies = GROWTH_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置自我成长内在力量留言:", growthReplies);
        setReplies(growthReplies);
      } else if (hasAmbitionKeywords) {
        const ambitionReplies = AMBITION_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置野心动力搞钱留言:", ambitionReplies);
        setReplies(ambitionReplies);
      } else if (hasNostalgiaKeywords) {
        const nostalgiaReplies = NOSTALGIA_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置怀旧回忆留言:", nostalgiaReplies);
        setReplies(nostalgiaReplies);
      } else if (hasInterpersonalKeywords) {
        const interpersonalReplies = INTERPERSONAL_REPLIES_TEMPLATES.map((reply) => ({
          ...reply,
          from: generateRandomStarNumber(),
          when: generateRandomTime()
        }));
        console.log("设置人际困扰留言:", interpersonalReplies);
  ...
  reactExports.useEffect(() => {
    ...
    console.log("更新similarCardCount:", cardCount, "当前留言数量:", replies.length);
  }, [replies]);
  reactExports.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative flex flex-col min-h-screen w-full overflow-hidden bg-background grain", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "relative z-10 flex-grow px-5 pb-20 pt-24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mx-auto max-w-md space-y-5", children: replies.map((r, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: "border border-white/10 bg-white/5 backdrop-blur-md p-5 rounded-lg animate-fade-up", style: {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between text-[10px] tracking-[0.25em] text-white", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: r.when })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-4 text-[15px] leading-[1.9] text-white font-cn", children: r.text }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-3 flex flex-wrap gap-2", children: r.tags.map((tag, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "border border-white/10 bg-white/5 backdrop-blur-sm rounded-full px-2 py-1 text-xs text-white/70", children: [
        "#",
        tag
      ] }, index)) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-5 pt-3 border-t border-border/50 flex items-center justify-between text-[11px] tracking-[0.25em]", children: [
        ...
        }, className: "text-white hover:text-white/80 transition-colors", children: [
          "同频",
          likedPosts.has(i) ? "❤️" : "🤍"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/message", search: {
          ...
          from: "similar",
          liked: void 0
        }, className: "text-white hover:text-white/80 transition-colors", children: "留言→" })
      ] })
    ] }, i)) }) })
  ] });
}
export {
  Similar as component
};
